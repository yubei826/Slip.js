...
(function (WIN, DOC) {
    ...
    var MIN_ALLOW_DISTANCE = 10;

    // 如果是单方向滑动 非滑动方向最大允许距离
    var MAX_OPP_ALLOW_DISTANCE = 40;
    ...
    var setTranslate = function (ele, x, y, z) {
        ...
        for (var i = 0, len = CSS_PREFIX_MAP.length; i < len; i++) {
            ...
            results.push(ele.style[name] = 'translate3d(' + (x || 0) + 'px, ' + (y || 0) + 'px, ' + (z || 0) + 'px)');
        }
        return results;
    };
    ...
    var Slip = (function () {
        ...
        /*
        * 构造器
        * @parm ele: 原生的dom元素，定义可以被滑动的元素
        * @parm direction: 可被滑动的方向，有三个合法值`'x'`, `'y'`, `'xy'`，默认为 `'x'`
        * */
        ...
        Slip.prototype.onTouchMove = function (event) {
            ...
            // 在元素上标记位移
            for (attr in eleMove) {
                results.push(ele.setAttribute(attr, eleMove[attr]));
            }
            return results;
        };
    ...
    var entry = function (ele, direction) {
        var instance = new Slip(ele, direction || X);
